...
async function openTeacherPhotoUpload(interaction) {
  ...
  console.log(
    `[TeacherPhoto] Waiting for ${interaction.user.id} to upload a photo for `
    + `${teacher.teacher_name} in channel ${interaction.channelId}.`
  );

  return respondInteraction(interaction, ephemeral(
    `📸 **Upload ${teacher.teacher_name}'s photo in this channel now.**\n\n`
    + 'Send **one image attachment** in your next message. Campus will save it automatically.\n'
    + 'This request expires in **5 minutes**.'
  ));
}

async function skipTeacherPhoto(interaction) {
  pendingTeacherPhotoUploads.delete(pendingTeacherPhotoKey(interaction.guildId, interaction.user.id));
...
client.on(Events.MessageCreate, async (message) => {
  ...
  // ----------------------------------------------------------
  // Discord photo messages can have no text content, so process them before
  // the AI/message.content checks.
  if (!message.author.bot) {
    ...
    if (pendingPhoto) {
      if (Date.now() > pendingPhoto.expiresAt) {
        ...
        console.log(`[TeacherPhoto] Upload request expired for user ${message.author.id}.`);
      } else if (String(message.channel.id) === String(pendingPhoto.channelId)) {
        const attachment = message.attachments?.find((item) => isImageAttachment(item));

        if (attachment) {
          try {
            console.log(
              `[TeacherPhoto] Image received for ${pendingPhoto.teacherName}: ${attachment.name || 'image'}`
            );
            ...
            await message.reply({
              ...
              console.warn('[TeacherPhoto] Photo saved, but confirmation reply failed:', error.message);
            });
            ...
            await message.reply({
              content:
                '❌ Campus received the image, but could not save it. '
                + 'Check the Render log for `[TeacherPhoto] Save error`.',
              allowedMentions: { repliedUser: false },
            }).catch(() => {});
            return;
          }
        ...
        if (message.attachments?.size) {
          await message.reply({
            content: '📸 That attachment does not look like an image. Please upload a PNG, JPG, GIF, or WEBP.',
            allowedMentions: { repliedUser: false },
          }).catch(() => {});
          return;
        }
