...
function teacherEditModal(teachers) {
  ...
  const details = new TextInputBuilder()
    .setCustomId('teacher_edit_details')
    .setPlaceholder('Favorites, hobbies, quirks, pet peeves, lore... Leave blank to keep current.')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(false)
    .setMaxLength(1000);
  ...
  return new ModalBuilder()
    ...
    .addLabelComponents(
      ...
      new LabelBuilder().setLabel('Favorites, Quirks + Details').setTextInputComponent(details),
    );
}
...
async function handleTeacherEditPickerModal(interaction) {
  ...
  const detailsRaw = interaction.fields.getTextInputValue('teacher_edit_details').trim();
  const selectedChannels = interaction.fields.getSelectedChannels(
