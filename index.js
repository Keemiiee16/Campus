...
async function openTeacherPhotoUpload(interaction) {
  ...
  pendingTeacherPhotoUploads.set(pendingTeacherPhotoKey(interaction.guildId, interaction.user.id), {
    schoolId: school.id,
...
client.on(Events.MessageCreate, async (message) => {
  ...
  // Teacher photo uploads are handled before AI conversation awareness so
  // uploading an avatar never creates a paid teacher response.
  if (!message.author.bot) {
    const uploadKey = pendingTeacherPhotoKey(message.guild.id, message.author.id);
    const pendingPhoto = pendingTeacherPhotoUploads.get(uploadKey);

    if (pendingPhoto) {
      if (Date.now() > pendingPhoto.expiresAt) {
        pendingTeacherPhotoUploads.delete(uploadKey);
      } else if (message.channel.id === pendingPhoto.channelId) {
        const attachment = message.attachments.find((item) => isImageAttachment(item));
        if (!attachment) {
          await message.reply({
            content: '📸 Upload a **PNG, JPG, GIF, or WEBP image attachment** for the teacher photo.',
        ...
        try {
          ...
          pendingTeacherPhotoUploads.delete(uploadKey);
          await message.reply({
            content: `✅ **${teacher.teacher_name}'s photo is saved!** Future teacher messages will use it.`,
            allowedMentions: { repliedUser: false },
          }).catch(() => {});
        } catch (error) {
          console.error('Teacher photo upload error:', error);
          await message.reply({
            content: '❌ I could not save that teacher photo. Check Render and try again.',
            allowedMentions: { repliedUser: false },
          }).catch(() => {});
        }
        return;
