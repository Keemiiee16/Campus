...
function cleanTeacherReply(teacher, content) {
  ...
  const escapedName = String(teacher?.teacher_name || '')
    .replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

  const labelPattern = escapedName
    ? new RegExp(`^\\s*(?:\\*\\*)?${escapedName}(?:\\*\\*)?\\s*:\\s*`, 'i')
    : null;

  const lines = text.split(/\r?\n/)
    .map((line) => labelPattern ? line.replace(labelPattern, '').trim() : line.trim())
    .filter(Boolean);

  if (!lines.length) return '';

  // Campus wants one teacher response per AI generation. If the model returns
  // multiple paraphrased answer lines, keep only the first one.
  return lines[0].trim();
}
...
async function sendAsTeacher(channel, teacher, content) {
  ...
  try {
    ...
    await channel.send({
      content: `**${teacher.teacher_name}:** ${cleanedContent}`,
      allowedMentions: { parse: [] },
    });
  }
}
...
async function openTeacherPhotoUpload(interaction) {
  // Acknowledge the button immediately so Discord cannot expire it while
  // Campus checks Supabase and creates the upload prompt.
  ...
  if (!interaction.channel?.isTextBased?.()) {
    return respondInteraction(interaction, ephemeral('Campus cannot create a photo upload prompt in this channel.'));
  }

  // Remove any older pending photo request for this same Discord user.
  const key = pendingTeacherPhotoKey(interaction.guildId, interaction.user.id);
  pendingTeacherPhotoUploads.delete(key);

  // This MUST be a normal channel message because Discord users cannot reply
  // to ephemeral messages with attachments.
  const promptMessage = await interaction.channel.send({
    content:
      `📸 **Teacher Photo — ${teacher.teacher_name}**\n`
      + `<@${interaction.user.id}>, **reply directly to this message** with one image attachment.\n`
      + 'PNG, JPG, GIF, or WEBP are accepted. This request expires in **5 minutes**.',
    allowedMentions: { users: [interaction.user.id] },
  });

  pendingTeacherPhotoUploads.set(key, {
    schoolId: school.id,
  ...
  console.log(
    `[TeacherPhoto] Reply prompt ${promptMessage.id} created for ${teacher.teacher_name} `
    + `by user ${interaction.user.id} in channel ${interaction.channelId}.`
  );

  return respondInteraction(interaction, ephemeral(
    `✅ Photo upload prompt posted for **${teacher.teacher_name}**.\n`
    + '**Reply directly to the Campus message in this channel with the teacher photo.**'
  ));
}

async function skipTeacherPhoto(interaction) {
  pendingTeacherPhotoUploads.delete(
    pendingTeacherPhotoKey(interaction.guildId, interaction.user.id)
  );

  return respondInteraction(
    interaction,
    ephemeral('✅ Teacher photo skipped for now. Any older photo-upload prompt can be ignored.')
  );
}
...
async function handleTeacherEditPickerModal(interaction) {
  ...
  return respondInteraction(interaction, {
    ...
    components: [
      personalDetailsButton(teacher.id, 'Edit Personal Details'),
      teacherPhotoButtons(refreshedTeacher || { ...teacher, teacher_name: changes.teacher_name }),
    ],
    flags: MessageFlags.Ephemeral,
  });
}
...
client.on(Events.MessageCreate, async (message) => {
  ...
  // ----------------------------------------------------------
  // TEACHER PHOTO — REPLY TO CAMPUS PROMPT
  // ----------------------------------------------------------
  // A photo upload can contain no text, so this runs before AI checks.
  if (!message.author.bot) {
    const photoKey = pendingTeacherPhotoKey(message.guild.id, message.author.id);
    const pendingPhoto = pendingTeacherPhotoUploads.get(photoKey);

    if (pendingPhoto) {
      if (Date.now() > pendingPhoto.expiresAt) {
        pendingTeacherPhotoUploads.delete(photoKey);

        console.log(
          `[TeacherPhoto] Reply prompt expired for ${pendingPhoto.teacherName} `
          + `and user ${message.author.id}.`
        );
      } else {
        const repliedToPrompt =
          String(message.reference?.messageId || '') === String(pendingPhoto.promptMessageId || '');
        ...
        if (repliedToPrompt) {
          ...
          if (!attachment) {
            await message.reply({
              content:
                '📸 Please reply to the Campus teacher-photo prompt with **one image attachment** '
                + '(PNG, JPG, GIF, or WEBP).',
          ...
          try {
            console.log(
              `[TeacherPhoto] Reply image received for ${pendingPhoto.teacherName}: `
              + `${attachment.name || 'image'} (${attachment.contentType || 'unknown type'})`
            );

            const { data: teacher, error: teacherError } = await supabase
              .from('teachers')
              .select('id, teacher_name')
              .eq('school_id', pendingPhoto.schoolId)
              .eq('id', pendingPhoto.teacherId)
              .maybeSingle();

            if (teacherError) throw teacherError;
            if (!teacher) throw new Error('Teacher no longer exists.');
            ...
            console.log(`[TeacherPhoto] Saved photo for ${teacher.teacher_name}.`);

            await message.reply({
              content:
                `✅ **${teacher.teacher_name}'s photo is saved!**\n`
                + 'Their future Campus teacher messages will use this picture.',
              allowedMentions: { repliedUser: false },
            }).catch((error) => {
              console.warn(
                '[TeacherPhoto] Photo saved, but Discord confirmation failed:',
                error.message
              );
            });
            ...
            console.error('[TeacherPhoto] Save error:', error);

            await message.reply({
              content:
                '❌ Campus received the teacher photo but could not save it. '
                + 'Check Render for `[TeacherPhoto] Save error`.',
              allowedMentions: { repliedUser: false },
            }).catch(() => {});
  ...
  try {
    ...
    // Direct questions / direct teacher mentions are never blocked by the
    // spontaneous-message cooldown. Cooldown only limits unsolicited chatter.
