...
async function sendAsTeacher(channel, teacher, content) {
  ...
  try {
    ...
    console.warn(
      `Webhook teacher send failed for ${teacher.teacher_name}; using Campus fallback:`,
      error.message
    );
    await channel.send({
      content: cleanedContent,
...
function extractRoleplayParts(content) {
  ...
  // Temporarily remove bold sections so their inner asterisks are not
  // accidentally treated like single-asterisk action/thought sections.
...
async function chooseRoamingTeacherForStudentMessage(school, message) {
  ...
  // Spoken dialogue naming a teacher chooses that specific teacher.
...
client.on(Events.MessageCreate, async (message) => {
  ...
  try {
    ...
    // 1) spoken dialogue is actually directed at them,
