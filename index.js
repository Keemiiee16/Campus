...
async function handleCampus(interaction) {
  ...
  try {
    ...
    return interaction.reply(ephemeral('That timezone is not valid. Example: `America/Chicago`.'));
  }
...
async function handleTeacher(interaction) {
  ...
  if (sub === 'create') {
    const payload = {
      school_id: school.id,
      teacher_name: interaction.options.getString('name', true),
      subject: interaction.options.getString('subject', true),
      grade_level: interaction.options.getString('grade'),
      classroom_channel_id: interaction.options.getChannel('classroom', true).id,
      personality: interaction.options.getString('personality', true),
      teaching_style: interaction.options.getString('teaching_style'),
      strictness: interaction.options.getInteger('strictness') ?? 5,
      talkativeness: interaction.options.getInteger('talkativeness') ?? 5,
      humor: interaction.options.getInteger('humor') ?? 5,
      patience: interaction.options.getInteger('patience') ?? 5,
      status: 'active',
    };
    ...
    return interaction.reply(ephemeral(
      `✅ **${data.teacher_name}** was created and assigned to <#${data.classroom_channel_id}>.\nTheir automation settings were created automatically.`
    ));
  }
  ...
  if (sub === 'edit') {
    ...
    for (const [option, column] of pairs) {
      const value = interaction.options.getString(option);
      if (value !== null) changes[column] = value;
    }
    const classroom = interaction.options.getChannel('classroom');
    if (classroom) changes.classroom_channel_id = classroom.id;
    ...
    return interaction.reply(ephemeral(`✅ Updated **${teacher.teacher_name}** without erasing their history or memories.`));
  }
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isModalSubmit() && interaction.customId.startsWith('campus_activity_modal:')) {
