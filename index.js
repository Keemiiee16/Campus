...
// Pending teacher avatar uploads. Keyed by guild + Discord user.
const pendingTeacherPhotoUploads = new Map();

// Prevent one Discord message from causing more than one paid teacher AI generation.
// processedTeacherMessages remembers message+teacher pairs briefly, while
// teacherReplyInFlight stops spontaneous automation from overlapping a live reply.
const processedTeacherMessages = new Map();
const teacherReplyInFlight = new Set();

// Tracks the newest student/Tupper message a teacher has already responded to.
// Spontaneous automation is not allowed to re-answer that same old conversation.
const lastTeacherSourceMessage = new Map();

function claimTeacherMessage(teacherId, messageId) {
  const now = Date.now();
  const ttl = 10 * 60_000;

  for (const [key, timestamp] of processedTeacherMessages) {
    if (now - timestamp > ttl) processedTeacherMessages.delete(key);
  }

  const key = `${teacherId}:${messageId}`;
  if (processedTeacherMessages.has(key)) return false;
  processedTeacherMessages.set(key, now);
  return true;
}

const lastNpcAction = new Map();
const npcSpontaneousHistory = new Map();

const npcCommandJSON = new SlashCommandBuilder()
  .setName('npc')
  .setDescription('Create and manage NPC students.')
  .setDefaultMemberPermissions(PermissionFlagsBits.ModerateMembers)
  .addSubcommand((sub) => sub.setName('create').setDescription('Create an NPC student.'))
  .addSubcommand((sub) => sub.setName('list').setDescription('View all NPC students.'))
  .addSubcommand((sub) => sub.setName('edit').setDescription('Edit an NPC student.'))
  .addSubcommand((sub) => sub.setName('status').setDescription('Set an NPC student active, away, or paused.'))
  .addSubcommand((sub) => sub.setName('automation').setDescription('Configure NPC student automation.'))
  .addSubcommand((sub) => sub.setName('remove').setDescription('Archive or permanently delete an NPC student.'))
  .toJSON();
...
async function deferEphemeralIfNeeded(interaction) {
  if (!interaction.deferred && !interaction.replied) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral });
  }
}

function withTeacherScheduleCommands(commands) {
  const cloned = commands.map((command) => JSON.parse(JSON.stringify(command)));
  const teacherCommand = cloned.find((command) => command.name === 'teacher');
  if (!teacherCommand) return cloned;

  teacherCommand.options = Array.isArray(teacherCommand.options) ? teacherCommand.options : [];
  const extra = [
    { type: 1, name: 'schedule', description: 'Schedule a message from a teacher.' },
    { type: 1, name: 'schedule-list', description: 'View scheduled teacher messages.' },
    { type: 1, name: 'schedule-manage', description: 'Pause, resume, or delete a scheduled teacher message.' },
  ];

  const existing = new Set(teacherCommand.options.map((option) => option.name));
  for (const option of extra) {
    if (!existing.has(option.name)) teacherCommand.options.push(option);
  }
  return cloned;
}

async function registerCommands() {
  if (String(process.env.REGISTER_COMMANDS ?? 'true').toLowerCase() === 'false') {
    console.log('[Commands] REGISTER_COMMANDS=false, skipping command registration.');
    return;
  }

  const baseCommands = withTeacherScheduleCommands(
    commandJSON.filter((command) => command.name !== 'npc')
  );
  const allCommands = [...baseCommands, npcCommandJSON];

  const commandNames = allCommands.map((command) => `/${command.name}`).join(', ');

  // Keep global commands for long-term/multi-server availability.
  await client.application.commands.set(allCommands);
  console.log(`[Commands] Registered ${allCommands.length} global Campus commands: ${commandNames}`);

  // Also register the same commands directly in every server Campus is in.
  // Guild commands refresh immediately and avoid waiting on Discord's global cache.
  for (const guild of client.guilds.cache.values()) {
    try {
      ...
      console.log(
        `[Commands] Registered ${allCommands.length} commands directly in ${guild.name} (${guild.id}): ${commandNames}`
      );
      console.log(
        `[Commands] Discord confirms guild commands in ${guild.name}: ${registeredNames}`
      );
      console.log(
        `[Commands] Discord confirms /routes in ${guild.name}: ${registered.some((command) => command.name === 'routes') ? 'YES' : 'NO'}`
      );
    } catch (error) {
      console.error(
        `[Commands] Guild registration failed for ${guild.name} (${guild.id}):`,
        error
      );
    }
  }
...
function cleanTeacherReply(teacher, content) {
  let text = String(content || '').trim();
  if (!text) return '';

  const teacherName = String(teacher?.teacher_name || '').trim();
  const escapedName = teacherName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

  if (escapedName) {
    const labelPatterns = [
      new RegExp(`^\\s*(?:["“”']\\s*)?(?:\\*\\*|__)?${escapedName}(?:\\*\\*|__)?\\s*[:\\-–—]\\s*`, 'i'),
      new RegExp(`^\\s*(?:["“”']\\s*)?(?:\\*\\*|__)?${escapedName}(?:\\*\\*|__)?\\s*\\n+`, 'i'),
    ];
...
async function sendAsTeacher(channel, teacher, content) {
  const cleanedContent = cleanTeacherReply(teacher, content);
  if (!cleanedContent) return;

  try {
    const webhookChannel = channel.isThread?.() ? channel.parent : channel;
    if (!webhookChannel?.fetchWebhooks || !webhookChannel?.createWebhook) {
      throw new Error('No webhook-capable parent channel.');
    }

    const webhooks = await webhookChannel.fetchWebhooks();
    let webhook = webhooks.find(
      (w) => w.owner?.id === client.user.id && w.name.startsWith('Campus Teacher')
    );
    ...
    console.error(
      `[TeacherWebhook] Failed to send as ${teacher.teacher_name}. Teacher reply was NOT posted as Campus:`,
      error
    );
    return null;
  }
...
function routesSetupModal() {
  const groupSelect = new StringSelectMenuBuilder()
    ...
    .addOptions(
      ...
      new StringSelectMenuOptionBuilder().setLabel('Assignments').setValue('assignments'),
...
async function handleRoutes(interaction) {
  ...
  if (!canManageRoutes) {
    return interaction.reply(ephemeral(
      'You need **Manage Channels**, **Manage Server**, Administrator, or server ownership to set up Campus routes.'
    ));
  }

  const school = await getSchool(interaction.guildId);
  if (!school) {
    return interaction.reply(ephemeral('Campus has not been set up for this server yet.'));
  }
...
async function saveRouteCategory(school, group, category) {
  const { error: groupError } = await supabase.from('channel_groups').upsert({
    ...
    onConflict: 'school_id,discord_category_id,group_type',
  });
  ...
  for (const child of children.values()) {
    ...
    const { error } = await supabase.from('channel_routes').upsert({
      school_id: school.id,
      discord_channel_id: child.id,
      discord_category_id: category.id,
      channel_name: child.name,
      route_type: routeType,
      active: true,
    }, {
      onConflict: 'school_id,discord_channel_id,route_type',
    });

    if (error) throw error;
    savedCount += 1;

    // Save all active/accessible threads under this channel as routes too.
    // That lets classroom threads appear as Assignment destinations.
    const threadMap = new Map();

    // 1) Threads already visible in the guild cache.
    for (const candidate of child.guild.channels.cache.values()) {
      if (
        candidate.isThread?.()
        && String(candidate.parentId) === String(child.id)
        && [
    ...
    // 2) Ask Discord directly for active threads under the parent channel.
    try {
      const active = await child.threads?.fetchActive?.();
      const fetchedThreads = active?.threads ?? active;

      if (fetchedThreads?.values) {
        for (const thread of fetchedThreads.values()) {
          if ([
            ChannelType.PublicThread,
            ChannelType.PrivateThread,
            ChannelType.AnnouncementThread,
      ...
      console.warn(
        `[Routes] Could not fetch active threads under #${child.name}:`,
        threadError?.message || threadError
      );
    }

    for (const thread of threadMap.values()) {
      const { error: threadRouteError } = await supabase.from('channel_routes').upsert({
        school_id: school.id,
      ...
      if (threadRouteError) throw threadRouteError;
      savedCount += 1;
    }
  }
...
async function handleRoutesSetupModal(interaction) {
  // Route setup can scan/save many channels and easily take longer than
  // Discord's interaction window. Acknowledge immediately, then edit the reply.
  await deferEphemeralIfNeeded(interaction);

  const school = await getSchool(interaction.guildId);
  if (!school) {
    return interaction.editReply('Campus has not been set up for this server yet.');
  }

  const groupValues = interaction.fields.getStringSelectValues('route_group');
  const group = groupValues?.[0];
  const validGroups = ['classrooms', 'assignments', 'locations', 'clubs', 'discipline'];

  if (!validGroups.includes(group)) {
    return interaction.editReply('Choose a valid route type.');
  }
  ...
  if (!categories.length) {
    return interaction.editReply('Choose at least one Discord category.');
  }
  ...
  if (!savedNames.length) {
    return interaction.editReply('I could not use any of the selected categories.');
  }
  ...
  return interaction.editReply(
    ...
    + `🔗 Campus found **${totalChannels} usable channels/threads** across them.\n\n`
...
function teacherPhotoButtons(teacher) {
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`campus_teacher_photo_add:${teacher.id}`)
      .setLabel(teacher.avatar_url ? 'Change Teacher Photo' : 'Add Teacher Photo')
      .setStyle(ButtonStyle.Primary),
    new ButtonBuilder()
      .setCustomId(`campus_teacher_photo_skip:${teacher.id}`)
  ...
  if (teacher.avatar_url) {
    row.addComponents(
      new ButtonBuilder()
        .setCustomId(`campus_teacher_photo_remove:${teacher.id}`)
        .setLabel('Remove Photo')
...
function pendingTeacherPhotoKey(guildId, userId) {
...
function isImageAttachment(attachment) {
  ...
  return /\.(png|jpe?g|gif|webp)$/i.test(String(attachment?.name || ''));
}

function teacherPhotoUploadModal(teacher) {
  const fileUpload = new FileUploadBuilder()
    .setCustomId('teacher_photo_file')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1);

  return new ModalBuilder()
    .setCustomId(`campus_teacher_photo_modal:${teacher.id}`)
    .setTitle(`Teacher Photo • ${String(teacher.teacher_name || 'Teacher').slice(0, 35)}`)
    .addLabelComponents(
      new LabelBuilder()
        .setLabel('Upload Teacher Photo')
        .setDescription('Choose one image file for this teacher.')
        .setFileUploadComponent(fileUpload)
    );
}

async function openTeacherPhotoUpload(interaction) {
  const school = await getSchool(interaction.guildId);
  if (!school) {
    return interaction.reply(ephemeral('Campus has not been set up for this server yet.'));
  }

  const teacherId = interaction.customId.match(/^campus_teacher_photo_add:(.+)$/)?.[1];
  if (!teacherId) return;

  const { data: teacher, error } = await supabase
    .from('teachers')
    .select('id, teacher_name, avatar_url')
  ...
  // IMPORTANT: showModal must be the FIRST acknowledgement of this button interaction.
  return interaction.showModal(teacherPhotoUploadModal(teacher));
}

async function handleTeacherPhotoUploadModal(interaction) {
  await deferEphemeralIfNeeded(interaction);
  ...
  const teacherId = interaction.customId.match(/^campus_teacher_photo_modal:(.+)$/)?.[1];
  if (!teacherId) return interaction.editReply('I could not determine which teacher this photo belongs to.');
  ...
  if (!teacher) {
    return interaction.editReply('I could not find that teacher anymore.');
  }

  const uploadedFiles = interaction.fields.getUploadedFiles('teacher_photo_file', true);
  const attachment = uploadedFiles?.first?.() ?? null;

  if (!attachment || !isImageAttachment(attachment)) {
    return interaction.editReply(
      'Please upload one image file (PNG, JPG, JPEG, GIF, or WEBP). Other file types are not accepted.'
    );
  }

  const { error: updateError } = await supabase
    .from('teachers')
    .update({ avatar_url: attachment.url })
    .eq('id', teacher.id);

  if (updateError) throw updateError;

  console.log(`[TeacherPhoto] Saved modal photo for ${teacher.teacher_name}: ${attachment.url}`);

  const refreshedTeacher = { ...teacher, avatar_url: attachment.url };

  return interaction.editReply({
    content:
      `✅ **${teacher.teacher_name}'s photo is saved!**\n`
      + 'Future teacher messages will use this picture.',
    components: [teacherPhotoButtons(refreshedTeacher)],
  });
}

async function skipTeacherPhoto(interaction) {
  return respondInteraction(interaction, ephemeral('✅ Teacher photo skipped for now.'));
}

async function removeTeacherPhoto(interaction) {

  const school = await requireSchool(interaction);
  if (!school) return;
  const teacherId = interaction.customId.match(/^campus_teacher_photo_remove:(.+)$/)?.[1];
  if (!teacherId) return;
  ...
  const { error } = await supabase.from('teachers').update({ avatar_url: null }).eq('id', teacher.id);
  if (error) throw error;
  return respondInteraction(interaction, ephemeral(`🗑️ Removed **${teacher.teacher_name}'s** teacher photo.`));
}
...
async function handleTeacherModal(interaction) {
  ...
  const { data: refreshedTeacher } = await supabase
    .from('teachers')
    .select('id, teacher_name, avatar_url')
    .eq('id', oldTeacher.id)
    .maybeSingle();
  ...
  return respondInteraction(interaction, {
    content:
      ...
      + 'You can edit their personal details or update their teacher photo below.',
    components: [
      personalDetailsButton(oldTeacher.id, 'Edit Personal Details'),
      teacherPhotoButtons(refreshedTeacher || oldTeacher),
    ],
    flags: MessageFlags.Ephemeral,
  });
}
...
async function handleTeacherPersonalDetailsModal(interaction) {
  ...
  const { data: updatedTeacher, error: refreshedError } = await supabase
    .from('teachers')
    .select('id, teacher_name, avatar_url')
    .eq('id', teacher.id)
    .maybeSingle();
  ...
  return respondInteraction(interaction, {
    content:
      ...
      + '📸 **Last step:** Would you like to add a teacher photo?',
    components: [teacherPhotoButtons(updatedTeacher || teacher)],
    flags: MessageFlags.Ephemeral,
  });
}
...
async function openTeacherPersonalDetails(interaction) {
  ...
  // IMPORTANT: do not defer/reply before opening a modal.
...
async function openTeacherRoamingSettings(interaction) {
  ...
  // A modal must be Discord's FIRST response to this button interaction.
  // Do not wait on Supabase here or Discord can expire the interaction token.
  // The submit handler validates the teacher against Supabase before saving.
  return interaction.showModal(teacherRoamingModal({ id: teacherId }, null));
}
...
async function openTeacherAutomationRedo(interaction) {
  ...
  // Open immediately; validate the teacher/scope on modal submit.
...
async function handleTeacher(interaction) {
  ...
  if (sub === 'schedule-list') {
    return handleTeacherScheduleList(interaction, school);
  }
  ...
  if (sub === 'schedule') {
    return interaction.showModal(teacherScheduleModal(teachers));
  }

  if (sub === 'schedule-manage') {
    ...
    if (error) throw error;
    if (!rows?.length) {
      return respondInteraction(interaction, ephemeral('No scheduled teacher messages yet. Use `/teacher schedule` first.'));
    }
    return interaction.showModal(teacherScheduleManageModal(rows));
  }
}
...
// ============================================================
// TEACHER SCHEDULED MESSAGES
// ============================================================

const teacherScheduleWeekdays = {
  sunday: 0,
  monday: 1,
  tuesday: 2,
  wednesday: 3,
...
function parseTeacherScheduleClock(raw) {
...
function parseTeacherScheduleWhen(type, raw) {
  const value = String(raw || '').trim();

  if (type === 'one_time') {
    const match = value.match(/^(\d{4}-\d{2}-\d{2})\s+(.+)$/);
    if (!match || !validDate(match[1])) return null;
    const time = parseTeacherScheduleClock(match[2]);
    return time ? { date: match[1], time } : null;
  }

  if (type === 'weekly') {
    const match = value.match(/^(Sunday|Monday|Tuesday|Wednesday|Thursday|Friday|Saturday)\s+(.+)$/i);
    if (!match) return null;
    const time = parseTeacherScheduleClock(match[2]);
    return time ? { weekday: match[1].toLowerCase(), time } : null;
  }

  if (type === 'daily' || type === 'weekdays') {
    const time = parseTeacherScheduleClock(value);
...
function teacherScheduleModal(teachers) {
  const teacherSelect = new StringSelectMenuBuilder()
    .setCustomId('teacher_schedule_teacher')
    .setPlaceholder('Choose the teacher')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(teacherChoiceOptions(teachers));
  ...
  const typeSelect = new StringSelectMenuBuilder()
    .setCustomId('teacher_schedule_type')
    .setPlaceholder('Choose how often it should send')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(
      new StringSelectMenuOptionBuilder().setLabel('One Time').setValue('one_time'),
      new StringSelectMenuOptionBuilder().setLabel('Every School Day').setValue('weekdays'),
      new StringSelectMenuOptionBuilder().setLabel('Every Day').setValue('daily'),
      new StringSelectMenuOptionBuilder().setLabel('Weekly').setValue('weekly'),
    );

  const whenInput = new TextInputBuilder()
    .setCustomId('teacher_schedule_when')
    .setPlaceholder('Examples: 8:05 AM | Monday 1:30 PM | 2026-09-01 9:00 AM')
    .setStyle(TextInputStyle.Short)
    .setRequired(true)
    .setMaxLength(80);

  const channelSelect = new ChannelSelectMenuBuilder()
    .setCustomId('teacher_schedule_channel')
    .setPlaceholder('Choose where the teacher should say it')
    .setChannelTypes(
      ChannelType.GuildText,
      ChannelType.GuildAnnouncement,
      ChannelType.PublicThread,
  ...
  const messageInput = new TextInputBuilder()
    .setCustomId('teacher_schedule_message')
    .setPlaceholder('What should the teacher say?')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(true)
    .setMaxLength(1800);

  return new ModalBuilder()
    .setCustomId('campus_teacher_schedule')
    .setTitle('Schedule Teacher Message')
    .addLabelComponents(
      new LabelBuilder().setLabel('Teacher').setStringSelectMenuComponent(teacherSelect),
      new LabelBuilder().setLabel('Schedule Type').setStringSelectMenuComponent(typeSelect),
      new LabelBuilder()
        .setLabel('When')
        .setDescription('One Time: YYYY-MM-DD 9:00 AM • Weekly: Monday 1:30 PM • Daily/School Day: 8:05 AM')
        .setTextInputComponent(whenInput),
      new LabelBuilder().setLabel('Channel / Thread').setChannelSelectMenuComponent(channelSelect),
      new LabelBuilder().setLabel('Message').setTextInputComponent(messageInput),
    );
}

async function handleTeacherScheduleModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;

  const teacherId = interaction.fields.getStringSelectValues('teacher_schedule_teacher')?.[0];
  const type = interaction.fields.getStringSelectValues('teacher_schedule_type')?.[0];
  const whenRaw = interaction.fields.getTextInputValue('teacher_schedule_when').trim();
  const message = interaction.fields.getTextInputValue('teacher_schedule_message').trim();
  const selected = interaction.fields.getSelectedChannels('teacher_schedule_channel', true);
  const channel = selected?.first() ?? null;

  const parsed = parseTeacherScheduleWhen(type, whenRaw);
  if (!parsed) {
    return respondInteraction(interaction, ephemeral(
      'I could not understand **When**.\n'
      + '• One Time: `2026-09-01 9:00 AM`\n'
      + '• Every School Day / Every Day: `8:05 AM`\n'
      + '• Weekly: `Monday 1:30 PM`'
    ));
  }

  if (!isTeacherAssignableChannel(channel)) {
    return respondInteraction(interaction, ephemeral('Choose a usable text channel or thread.'));
  }
  ...
  const { error } = await supabase.from('teacher_schedules').insert({
    ...
    schedule_value: whenRaw,
    time_of_day: parsed.time,
    scheduled_date: parsed.date || null,
    weekday_name: parsed.weekday || null,
  ...
  return respondInteraction(interaction, ephemeral(
    `✅ Scheduled **${teacher.teacher_name}** to speak in <#${channel.id}>.\n`
    + `🕒 **${type.replaceAll('_', ' ')}:** ${whenRaw}\n`
    + `💬 ${message}`
  ));
}

async function handleTeacherScheduleList(interaction, school) {
  const { data, error } = await supabase
    .from('teacher_schedules')
    .select('*')
    .eq('school_id', school.id)
  ...
  if (!data?.length) {
    return respondInteraction(interaction, ephemeral('No scheduled teacher messages yet. Use `/teacher schedule`.'));
  }

  const lines = data.map((row, index) => {
    const state = row.enabled ? '🟢 ON' : '⏸️ PAUSED';
    const when = row.recurrence_type === 'one_time'
      ? `${row.scheduled_date} ${row.time_of_day}`
      : row.recurrence_type === 'weekly'
        ? `${row.weekday_name} ${row.time_of_day}`
        : `${row.recurrence_type.replaceAll('_', ' ')} ${row.time_of_day}`;
    return `**${index + 1}. ${row.teacher_name}** — ${state}\n`
      + `↳ ${when} • <#${row.discord_channel_id}>\n`
      + `↳ ${String(row.message_text).slice(0, 220)}`;
  });

  return respondInteraction(interaction, ephemeral(
    `🗓️ **Scheduled Teacher Messages**\n\n${lines.join('\n\n')}`
  ));
}

function teacherScheduleManageModal(rows) {
  const scheduleSelect = new StringSelectMenuBuilder()
    .setCustomId('teacher_schedule_manage_target')
    .setPlaceholder('Choose a scheduled message')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(rows.slice(0, 25).map((row) =>
      new StringSelectMenuOptionBuilder()
        .setLabel(`${row.teacher_name} — ${String(row.schedule_value).slice(0, 60)}`.slice(0, 100))
        .setDescription(String(row.message_text).slice(0, 100))
        .setValue(String(row.id))
    ));

  const actionSelect = new StringSelectMenuBuilder()
    .setCustomId('teacher_schedule_manage_action')
    .setPlaceholder('Choose what to do')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(
      new StringSelectMenuOptionBuilder().setLabel('Pause').setValue('pause'),
      new StringSelectMenuOptionBuilder().setLabel('Resume').setValue('resume'),
      new StringSelectMenuOptionBuilder().setLabel('Delete').setValue('delete'),
    );

  return new ModalBuilder()
    .setCustomId('campus_teacher_schedule_manage')
    .setTitle('Manage Teacher Schedule')
    .addLabelComponents(
      new LabelBuilder().setLabel('Scheduled Message').setStringSelectMenuComponent(scheduleSelect),
      new LabelBuilder().setLabel('Action').setStringSelectMenuComponent(actionSelect),
    );
}

async function handleTeacherScheduleManageModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;
  const id = interaction.fields.getStringSelectValues('teacher_schedule_manage_target')?.[0];
  const action = interaction.fields.getStringSelectValues('teacher_schedule_manage_action')?.[0];
  ...
  if (action === 'delete') {
    const { error } = await supabase.from('teacher_schedules')
      ...
      .eq('id', id);
    if (error) throw error;
    return respondInteraction(interaction, ephemeral('🗑️ Scheduled teacher message deleted.'));
  }
  ...
  return respondInteraction(interaction, ephemeral(
    enabled ? '▶️ Scheduled teacher message resumed.' : '⏸️ Scheduled teacher message paused.'
  ));
}

async function teacherScheduleTick() {
  ...
  const { data: rows, error } = await supabase
    ...
    .eq('enabled', true);
  if (error) {
    console.error('Teacher schedule tick DB error:', error.message);
    return;
  }
  ...
  for (const row of rows ?? []) {
    try {
      const { data: school, error: schoolError } = await supabase
        ...
        .eq('id', row.school_id)
        .maybeSingle();
      if (schoolError || !school) continue;

      const local = getLocalParts(school.timezone);
      if (local.time !== String(row.time_of_day || '').slice(0, 5)) continue;
      ...
      if (row.recurrence_type === 'one_time') {
        shouldSend = today === row.scheduled_date;
      } else if (row.recurrence_type === 'daily') {
        shouldSend = true;
      } else if (row.recurrence_type === 'weekdays') {
        shouldSend = await isOpenSchoolDate(school, today);
      } else if (row.recurrence_type === 'weekly') {
        shouldSend = teacherScheduleWeekdays[String(row.weekday_name || '').toLowerCase()] === weekdayIndex
          && await isOpenSchoolDate(school, today);
      }

      if (!shouldSend) continue;

      const sendKey = `${today}:${row.time_of_day}`;
      ...
      const { data: teacher, error: teacherError } = await supabase
        ...
        .maybeSingle();
      if (teacherError || !teacher || ['away', 'paused'].includes(teacher.status)) continue;

      const guild = client.guilds.cache.get(school.discord_guild_id);
      if (!guild) continue;
      const channel = await guild.channels.fetch(row.discord_channel_id).catch(() => null);
      if (!channel?.isTextBased()) continue;

      await sendAsTeacher(channel, teacher, row.message_text);

      await supabase.from('teacher_schedules')
        .update({
          last_sent_key: sendKey,
          last_sent_at: new Date().toISOString(),
          enabled: row.recurrence_type === 'one_time' ? false : true,
        })
        .eq('id', row.id);
    } catch (error) {
      console.error(`Teacher schedule ${row.id} error:`, error.message);
    }
  }
}
...
function npcPersonalDetailsModal(npc) {
  const current = parseTeacherPersonalDetails(npc?.lore);
  const makeInput = (id, placeholder, value = '') => {
    const input = new TextInputBuilder()
      .setCustomId(id)
      .setPlaceholder(placeholder)
  ...
  return new ModalBuilder()
    .setCustomId(`campus_npc_personal:${npc.id}`)
    .setTitle('NPC Student Personal Details')
    .addLabelComponents(
      new LabelBuilder().setLabel('Favorites').setTextInputComponent(
        makeInput('npc_favorites', 'Favorite foods, colors, music, classes, sports...', current.favorites)
      ),
      new LabelBuilder().setLabel('Quirks').setTextInputComponent(
        makeInput('npc_quirks', 'Habits, mannerisms, little funny behaviors...', current.quirks)
      ),
      new LabelBuilder().setLabel('Pet Peeves').setTextInputComponent(
        makeInput('npc_pet_peeves', 'Things that annoy this student...', current.petPeeves)
      ),
      new LabelBuilder().setLabel('Likes').setTextInputComponent(
        makeInput('npc_likes', 'Things this student enjoys...', current.likes)
      ),
      new LabelBuilder().setLabel('Dislikes').setTextInputComponent(
        makeInput('npc_dislikes', 'Things this student dislikes...', current.dislikes)
      ),
    );
}

function npcPersonalDetailsButton(npcId, label = 'Add Personal Details') {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`campus_npc_personal_open:${npcId}`)
      .setLabel(label)
      .setStyle(ButtonStyle.Secondary)
  );
}

function npcRoamingButton(npcId) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`campus_npc_roaming_open:${npcId}`)
      .setLabel('Roaming & Common Areas')
      .setStyle(ButtonStyle.Primary)
  );
}

function npcCreateModal() {
  const name = new TextInputBuilder()
    .setCustomId('npc_name')
    .setPlaceholder('Avery Brooks')
    .setStyle(TextInputStyle.Short)
    .setRequired(true)
    .setMaxLength(80);

  const grade = new TextInputBuilder()
    .setCustomId('npc_grade')
    .setPlaceholder('5th Grade')
    .setStyle(TextInputStyle.Short)
    .setRequired(true)
    .setMaxLength(80);

  const channel = new ChannelSelectMenuBuilder()
    .setCustomId('npc_channel')
    .setPlaceholder('Choose their main classroom or hangout channel')
    .setChannelTypes(
      ChannelType.GuildText,
      ChannelType.GuildAnnouncement,
      ChannelType.PublicThread,
  ...
  const personality = new TextInputBuilder()
    .setCustomId('npc_personality')
    .setPlaceholder('Quiet at first, funny with friends, competitive, kind...')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(true)
    .setMaxLength(1000);

  const interests = new TextInputBuilder()
    .setCustomId('npc_interests')
    .setPlaceholder('Dance, robotics, basketball, art, gossip, gaming...')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(false)
    .setMaxLength(1000);

  return new ModalBuilder()
    .setCustomId('campus_npc_create')
    .setTitle('Create NPC Student')
    .addLabelComponents(
      new LabelBuilder().setLabel('Student Name').setTextInputComponent(name),
      new LabelBuilder().setLabel('Grade').setTextInputComponent(grade),
      new LabelBuilder().setLabel('Assigned Channel').setChannelSelectMenuComponent(channel),
      new LabelBuilder().setLabel('Personality').setTextInputComponent(personality),
      new LabelBuilder().setLabel('Interests / Hobbies').setTextInputComponent(interests),
    );
}

async function getNpcChoices(schoolId) {
  const { data, error } = await supabase
    .from('npc_students')
    .select('id, student_name, grade_level, status, assigned_channel_id')
    .eq('school_id', schoolId)
    .neq('status', 'archived')
    .order('student_name')
    .limit(25);
...
function npcChoiceOptions(npcs) {
  return npcs.map((npc) => {
    const option = new StringSelectMenuOptionBuilder()
      .setLabel(String(npc.student_name).slice(0, 100))
      .setValue(String(npc.id));
    const description = [npc.grade_level, npc.status].filter(Boolean).join(' • ').slice(0, 100);
    if (description) option.setDescription(description);
    return option;
  });
}

function npcEditModal(npcs) {
  const target = new StringSelectMenuBuilder()
    .setCustomId('npc_edit_target')
    .setPlaceholder('Choose the NPC student')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(npcChoiceOptions(npcs));

  const identity = new TextInputBuilder()
    .setCustomId('npc_edit_identity')
    .setPlaceholder('Avery Brooks | 5th Grade')
    .setStyle(TextInputStyle.Short)
    .setRequired(false)
    .setMaxLength(180);

  const channel = new ChannelSelectMenuBuilder()
    .setCustomId('npc_edit_channel')
    .setPlaceholder('Choose a new main channel, or leave blank')
    .setChannelTypes(
      ChannelType.GuildText,
      ChannelType.GuildAnnouncement,
      ChannelType.PublicThread,
  ...
  const personality = new TextInputBuilder()
    .setCustomId('npc_edit_personality')
    .setPlaceholder('Leave blank to keep the current personality.')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(false)
    .setMaxLength(1000);
  ...
  const interests = new TextInputBuilder()
    .setCustomId('npc_edit_interests')
    .setPlaceholder('Leave blank to keep current interests.')
    .setStyle(TextInputStyle.Paragraph)
    .setRequired(false)
    .setMaxLength(1000);

  return new ModalBuilder()
    .setCustomId('campus_npc_edit')
    .setTitle('Edit NPC Student')
    .addLabelComponents(
      new LabelBuilder().setLabel('Which NPC Student?').setStringSelectMenuComponent(target),
      new LabelBuilder().setLabel('Name | Grade').setDescription('Optional. Leave blank to keep both.').setTextInputComponent(identity),
      new LabelBuilder().setLabel('Assigned Channel').setDescription('Optional. Leave blank to keep current channel.').setChannelSelectMenuComponent(channel),
      new LabelBuilder().setLabel('Personality').setTextInputComponent(personality),
      new LabelBuilder().setLabel('Interests / Hobbies').setTextInputComponent(interests),
    );
}

function npcStatusModal(npcs) {
  const target = new StringSelectMenuBuilder()
    .setCustomId('npc_status_target')
    .setPlaceholder('Choose an NPC student')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(npcChoiceOptions(npcs));
  ...
  return new ModalBuilder()
    .setCustomId('campus_npc_status')
    .setTitle('NPC Student Status')
    .addLabelComponents(
      new LabelBuilder().setLabel('NPC Student').setStringSelectMenuComponent(target),
      new LabelBuilder().setLabel('New Status').setStringSelectMenuComponent(status),
    );
}

function npcAutomationModal(npcs) {
  const target = new StringSelectMenuBuilder()
    .setCustomId('npc_automation_target')
    .setPlaceholder('Choose an NPC student')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
  ...
  const enabled = new StringSelectMenuBuilder()
    .setCustomId('npc_automation_enabled')
    .setPlaceholder('Turn automation on or off')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
  ...
  const frequency = new StringSelectMenuBuilder()
    .setCustomId('npc_automation_frequency')
    .setPlaceholder('How active should this student be?')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(
  ...
  const behaviors = new StringSelectMenuBuilder()
    .setCustomId('npc_automation_behaviors')
    .setPlaceholder('Choose behaviors that are ON')
    .setRequired(false)
    .setMinValues(0)
    .setMaxValues(3)
    .addOptions(
  ...
  const cooldown = new StringSelectMenuBuilder()
    .setCustomId('npc_automation_cooldown')
    .setPlaceholder('Minimum cooldown')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
  ...
  return new ModalBuilder()
    .setCustomId('campus_npc_automation')
    .setTitle('NPC Student Automation')
    .addLabelComponents(
      new LabelBuilder().setLabel('NPC Student').setStringSelectMenuComponent(target),
      new LabelBuilder().setLabel('Automation').setStringSelectMenuComponent(enabled),
      new LabelBuilder().setLabel('Activity Frequency').setStringSelectMenuComponent(frequency),
      new LabelBuilder().setLabel('Behaviors').setDescription('Unselected behaviors will be OFF.').setStringSelectMenuComponent(behaviors),
      new LabelBuilder().setLabel('Minimum Cooldown').setStringSelectMenuComponent(cooldown),
    );
}

function npcRoamingModal(npc, settings) {
  const enabled = new StringSelectMenuBuilder()
    .setCustomId('npc_roaming_enabled')
    .setPlaceholder('Turn roaming on or off')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
  ...
  const channels = new ChannelSelectMenuBuilder()
    .setCustomId('npc_roaming_channels')
    .setPlaceholder('Choose common areas this student can visit')
    .setRequired(false)
    .setMinValues(0)
    .setMaxValues(25)
    .setChannelTypes(
  ...
  const currentFrequency = settings?.roaming_frequency || 'normal';
  const frequency = new StringSelectMenuBuilder()
    .setCustomId('npc_roaming_frequency')
    .setPlaceholder('How often should this student show up there?')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(
  ...
  return new ModalBuilder()
    .setCustomId(`campus_npc_roaming:${npc.id}`)
    .setTitle('NPC Roaming & Common Areas')
    .addLabelComponents(
      new LabelBuilder().setLabel('Common-Area Roaming').setStringSelectMenuComponent(enabled),
      new LabelBuilder().setLabel('Common-Area Channels').setDescription('Cafeteria, halls, library, gym, clubs, etc.').setChannelSelectMenuComponent(channels),
      new LabelBuilder().setLabel('Roaming Frequency').setStringSelectMenuComponent(frequency),
    );
}

function npcRemoveModal(npcs) {
  const target = new StringSelectMenuBuilder()
    .setCustomId('npc_remove_target')
    .setPlaceholder('Choose an NPC student')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
  ...
  const mode = new StringSelectMenuBuilder()
    .setCustomId('npc_remove_mode')
    .setPlaceholder('Choose what Remove should do')
    .setRequired(true)
    .setMinValues(1)
    .setMaxValues(1)
    .addOptions(
      new StringSelectMenuOptionBuilder().setLabel('Archive / deactivate').setDescription('Keeps the profile but stops the NPC.').setValue('archive'),
      new StringSelectMenuOptionBuilder().setLabel('Permanently delete').setDescription('Deletes the NPC profile and settings.').setValue('delete'),
    );

  return new ModalBuilder()
    .setCustomId('campus_npc_remove')
    .setTitle('Remove NPC Student')
    .addLabelComponents(
      new LabelBuilder().setLabel('NPC Student').setStringSelectMenuComponent(target),
      new LabelBuilder().setLabel('Remove Option').setStringSelectMenuComponent(mode),
    );
}

async function handleNpcCreateModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;

  const studentName = interaction.fields.getTextInputValue('npc_name').trim();
  const gradeLevel = interaction.fields.getTextInputValue('npc_grade').trim();
  const selected = interaction.fields.getSelectedChannels('npc_channel', true);
  const channel = selected?.first() ?? null;
  const personality = interaction.fields.getTextInputValue('npc_personality').trim();
  const interests = interaction.fields.getTextInputValue('npc_interests').trim();

  if (!isTeacherAssignableChannel(channel)) {
    return respondInteraction(interaction, ephemeral('That assigned channel/thread no longer exists or Campus cannot use it.'));
  }
  ...
  if (existing) {
    return respondInteraction(interaction, ephemeral(`An NPC student named **${existing.student_name}** already exists. Use \`/npc edit\`.`));
  }

  const { data: npc, error } = await supabase.from('npc_students').insert({
    school_id: school.id,
  ...
  const { error: settingsError } = await supabase.from('npc_settings').insert({ npc_id: npc.id });
  if (settingsError) throw settingsError;

  return respondInteraction(interaction, {
    content:
      `✅ **${npc.student_name}** was created as an NPC student.\n`
      + `🎒 Grade: **${npc.grade_level}**\n`
      + `🏫 Main channel: <#${npc.assigned_channel_id}>\n`
      + `🟢 Status: **Active**\n\n`
      + 'Next, add their personal details and then use `/npc automation` to control how often they talk.',
    components: [npcPersonalDetailsButton(npc.id)],
    flags: MessageFlags.Ephemeral,
  });
}

async function openNpcPersonalDetails(interaction) {
  // Fetch before opening a modal, but fail quickly if Discord is close to expiring.
  const school = await getSchool(interaction.guildId);
  if (!school) return respondInteraction(interaction, ephemeral('Set up this school first with `/campus setup`.'));
  const npcId = interaction.customId.split(':')[1];
  const { data: npc, error } = await supabase.from('npc_students').select('*').eq('school_id', school.id).eq('id', npcId).maybeSingle();
  if (error) throw error;
  if (!npc) return respondInteraction(interaction, ephemeral('I could not find that NPC student anymore.'));
  return interaction.showModal(npcPersonalDetailsModal(npc));
}

async function handleNpcPersonalDetailsModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;
  const npcId = interaction.customId.split(':')[1];
  const details = {
    favorites: interaction.fields.getTextInputValue('npc_favorites').trim(),
    quirks: interaction.fields.getTextInputValue('npc_quirks').trim(),
    petPeeves: interaction.fields.getTextInputValue('npc_pet_peeves').trim(),
    likes: interaction.fields.getTextInputValue('npc_likes').trim(),
    dislikes: interaction.fields.getTextInputValue('npc_dislikes').trim(),
  };
  const lore = formatTeacherPersonalDetails(details);
  const { data: npc, error } = await supabase.from('npc_students')
    .update({ lore })
    .eq('school_id', school.id)
    .eq('id', npcId)
    .select('student_name')
  ...
  if (error) throw error;
  if (!npc) return respondInteraction(interaction, ephemeral('I could not find that NPC student anymore.'));
  return respondInteraction(interaction, ephemeral(`✅ Personal details saved for **${npc.student_name}**.`));
}

async function handleNpcEditModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;
  const npcId = interaction.fields.getStringSelectValues('npc_edit_target')?.[0];
  const { data: npc, error } = await supabase.from('npc_students').select('*').eq('school_id', school.id).eq('id', npcId).maybeSingle();
  if (error) throw error;
  if (!npc) return respondInteraction(interaction, ephemeral('I could not find that NPC student anymore.'));

  const identity = interaction.fields.getTextInputValue('npc_edit_identity').trim();
  let studentName = npc.student_name;
  let gradeLevel = npc.grade_level;
  if (identity) {
    const parts = identity.split('|').map((part) => part.trim());
    studentName = parts[0] || studentName;
    if (parts.length > 1) gradeLevel = parts.slice(1).join(' | ').trim() || gradeLevel;
  }

  const selected = interaction.fields.getSelectedChannels('npc_edit_channel', false);
  const channel = selected?.first() ?? null;
  const personality = interaction.fields.getTextInputValue('npc_edit_personality').trim();
  const interests = interaction.fields.getTextInputValue('npc_edit_interests').trim();
  if (channel && !isTeacherAssignableChannel(channel)) {
    return respondInteraction(interaction, ephemeral('That channel/thread cannot be used.'));
  }
  ...
  const changes = {
    ...
    grade_level: gradeLevel,
    assigned_channel_id: channel?.id || npc.assigned_channel_id,
    personality: personality || npc.personality,
    interests: interests || npc.interests,
  };
  const { error: updateError } = await supabase.from('npc_students').update(changes).eq('id', npc.id);
  if (updateError) throw updateError;

  return respondInteraction(interaction, {
    content: `✅ **${studentName}** was updated.\n🏫 Main channel: <#${changes.assigned_channel_id}>`,
    components: [npcPersonalDetailsButton(npc.id, 'Edit Personal Details')],
    flags: MessageFlags.Ephemeral,
  });
}

async function handleNpcStatusModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;
  const npcId = interaction.fields.getStringSelectValues('npc_status_target')?.[0];
  const status = interaction.fields.getStringSelectValues('npc_status_value')?.[0];
  if (!['active', 'away', 'paused'].includes(status)) return respondInteraction(interaction, ephemeral('Choose a valid NPC status.'));
  const { data: npc, error } = await supabase.from('npc_students')
    .update({ status })
    .eq('school_id', school.id)
    .eq('id', npcId)
  ...
  if (error) throw error;
  if (!npc) return respondInteraction(interaction, ephemeral('I could not find that NPC student anymore.'));
  const icon = status === 'active' ? '🟢' : status === 'away' ? '🟡' : '🔴';
  return respondInteraction(interaction, ephemeral(`${icon} **${npc.student_name}** is now **${status.replace('_', ' ')}**.`));
}

async function handleNpcAutomationModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;
  const npcId = interaction.fields.getStringSelectValues('npc_automation_target')?.[0];
  const enabled = interaction.fields.getStringSelectValues('npc_automation_enabled')?.[0] === 'on';
  const frequency = interaction.fields.getStringSelectValues('npc_automation_frequency')?.[0] || 'normal';
  const behaviors = new Set(interaction.fields.getStringSelectValues('npc_automation_behaviors') || []);
  const cooldown = Number(interaction.fields.getStringSelectValues('npc_automation_cooldown')?.[0] || 30);

  const { data: npc, error: npcError } = await supabase.from('npc_students').select('id, student_name').eq('school_id', school.id).eq('id', npcId).maybeSingle();
  if (npcError) throw npcError;
  if (!npc) return respondInteraction(interaction, ephemeral('I could not find that NPC student anymore.'));

  const { error } = await supabase.from('npc_settings').upsert({
    npc_id: npc.id,
    automation_enabled: enabled,
    activity_frequency: frequency,
    react_to_messages: behaviors.has('react'),
    spontaneous_messages: behaviors.has('spontaneous'),
    conversation_awareness: behaviors.has('conversation'),
  ...
  return respondInteraction(interaction, {
    content:
      `✅ NPC automation saved for **${npc.student_name}**.\n`
      + `🤖 Automation: **${enabled ? 'ON' : 'OFF'}**\n`
      + `📈 Frequency: **${frequency.replace('_', ' ')}**\n`
      + `⏱️ Cooldown: **${cooldown} minute${cooldown === 1 ? '' : 's'}**`,
    components: [npcRoamingButton(npc.id)],
    flags: MessageFlags.Ephemeral,
  });
}

async function openNpcRoaming(interaction) {
  const school = await getSchool(interaction.guildId);
  if (!school) return respondInteraction(interaction, ephemeral('Set up this school first with `/campus setup`.'));
  const npcId = interaction.customId.split(':')[1];
  const [{ data: npc, error: npcError }, { data: settings, error: settingsError }] = await Promise.all([
    supabase.from('npc_students').select('*').eq('school_id', school.id).eq('id', npcId).maybeSingle(),
    supabase.from('npc_settings').select('*').eq('npc_id', npcId).maybeSingle(),
  ]);
  if (npcError) throw npcError;
  if (settingsError) throw settingsError;
  if (!npc) return respondInteraction(interaction, ephemeral('I could not find that NPC student anymore.'));
  return interaction.showModal(npcRoamingModal(npc, settings));
}

async function handleNpcRoamingModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;
  const npcId = interaction.customId.split(':')[1];
  const enabled = interaction.fields.getStringSelectValues('npc_roaming_enabled')?.[0] === 'on';
  const frequency = interaction.fields.getStringSelectValues('npc_roaming_frequency')?.[0] || 'normal';
  const selected = interaction.fields.getSelectedChannels('npc_roaming_channels', false);
  const channelIds = [...(selected?.values() ?? [])]
    .filter(isTeacherAssignableChannel)
    .map((channel) => String(channel.id));
  ...
  if (error) throw error;
  return respondInteraction(interaction, ephemeral(
    `✅ Roaming saved for **${npc.student_name}**.\n`
    + `🚶 Roaming: **${enabled ? 'ON' : 'OFF'}**\n`
    + `📍 Common areas: **${channelIds.length}**\n`
    + `📈 Frequency: **${frequency}**`
  ));
}

async function handleNpcRemoveModal(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;
  const npcId = interaction.fields.getStringSelectValues('npc_remove_target')?.[0];
  const mode = interaction.fields.getStringSelectValues('npc_remove_mode')?.[0];
  const { data: npc, error: findError } = await supabase.from('npc_students').select('id, student_name').eq('school_id', school.id).eq('id', npcId).maybeSingle();
  if (findError) throw findError;
  if (!npc) return respondInteraction(interaction, ephemeral('I could not find that NPC student anymore.'));

  if (mode === 'delete') {
    const { error } = await supabase.from('npc_students').delete().eq('id', npc.id);
    if (error) throw error;
    return respondInteraction(interaction, ephemeral(`🗑️ **${npc.student_name}** was permanently deleted.`));
  }

  const { error } = await supabase.from('npc_students').update({ status: 'archived' }).eq('id', npc.id);
  if (error) throw error;
  return respondInteraction(interaction, ephemeral(`📦 **${npc.student_name}** was archived and will no longer appear or talk.`));
}

async function handleNpcList(interaction) {
  const school = await requireSchool(interaction);
  if (!school) return;
  const { data: npcs, error } = await supabase.from('npc_students')
    .select('id, student_name, grade_level, assigned_channel_id, status')
    .eq('school_id', school.id)
    .neq('status', 'archived')
    .order('student_name');
  if (error) throw error;
  if (!npcs?.length) return respondInteraction(interaction, ephemeral('No NPC students are saved yet. Use `/npc create` first.'));
  const lines = npcs.map((npc) => {
    const icon = npc.status === 'active' ? '🟢' : npc.status === 'away' ? '🟡' : '🔴';
    return `${icon} **${npc.student_name}** — ${npc.grade_level || 'Grade not set'} — <#${npc.assigned_channel_id}> — ${npc.status}`;
  });
  return respondInteraction(interaction, ephemeral(`🎒 **${school.school_name || 'Campus'} — NPC Students**\n\n${lines.join('\n')}`));
}
...
async function handleNpc(interaction) {
  ...
  if (!school) return;
  const sub = interaction.options.getSubcommand();
  if (sub === 'create') return interaction.showModal(npcCreateModal());
  if (sub === 'list') return handleNpcList(interaction);

  const npcs = await getNpcChoices(school.id);
  if (!npcs.length) return respondInteraction(interaction, ephemeral('No NPC students are saved yet. Use `/npc create` first.'));
  if (sub === 'edit') return interaction.showModal(npcEditModal(npcs));
  if (sub === 'status') return interaction.showModal(npcStatusModal(npcs));
  if (sub === 'automation') return interaction.showModal(npcAutomationModal(npcs));
  if (sub === 'remove') return interaction.showModal(npcRemoveModal(npcs));
}
...
function campusCommandCenterPayload() {
  const select = new StringSelectMenuBuilder()
    ...
    .addOptions(
      ...
      new StringSelectMenuOptionBuilder().setLabel('🎒 Create NPC Student').setValue('npc_create'),
      new StringSelectMenuOptionBuilder().setLabel('🧑‍🎓 View NPC Students').setValue('npc_list'),
      new StringSelectMenuOptionBuilder().setLabel('✏️ Edit NPC Student').setValue('npc_edit'),
      new StringSelectMenuOptionBuilder().setLabel('🟢 NPC Student Status').setValue('npc_status'),
      new StringSelectMenuOptionBuilder().setLabel('🤖 NPC Student Automation').setValue('npc_automation'),
      new StringSelectMenuOptionBuilder().setLabel('🗑️ Remove NPC Student').setValue('npc_remove'),
...
async function handleCampusCommandCenter(interaction) {
  ...
  if (choice === 'guide') {
    return respondInteraction(interaction, ephemeral(
      ...
      + '`/npc create` — create an AI NPC student\n'
      + '`/npc list` — view NPC students and statuses\n'
      + '`/npc edit` — edit an NPC student\n'
      + '`/npc status` — active, away, or paused\n'
      + '`/npc automation` — NPC conversation, reactions, spontaneous chat, and roaming\n'
      + '`/npc remove` — archive or delete an NPC student\n'
  ...
  if (choice === 'npc_create') return interaction.showModal(npcCreateModal());
  if (choice === 'npc_list') return handleNpcList(interaction);
  if (['npc_edit', 'npc_status', 'npc_automation', 'npc_remove'].includes(choice)) {
    const npcs = await getNpcChoices(school.id);
    if (!npcs.length) return respondInteraction(interaction, ephemeral('No NPC students are saved yet. Use `/npc create` first.'));
    if (choice === 'npc_edit') return interaction.showModal(npcEditModal(npcs));
    if (choice === 'npc_status') return interaction.showModal(npcStatusModal(npcs));
    if (choice === 'npc_automation') return interaction.showModal(npcAutomationModal(npcs));
    if (choice === 'npc_remove') return interaction.showModal(npcRemoveModal(npcs));
  }
...
async function handleActivityChoice(interaction) {
  ...
  const routeTypes = activity === 'assignment' ? 'assignment' : cfg.routes;
  const routes = await getRoutes(school.id, routeTypes);
  if (!routes.length) {
    return interaction.update({
      content: activity === 'assignment'
        ? 'I don\'t have an **Assignments** route saved yet. An admin should run `/routes setup` and choose **Assignments**.'
        : `I don't have any matching channels saved for **${cfg.label}** yet. An admin should run \`/routes setup\`.`,
      components: [],
    });
  }
...
async function handleDestinationChoice(interaction) {
  ...
  // Assignments can optionally include one submitted photo.
  // Do not use setFileTypes() here because the discord.js build currently
  // running Campus does not support that method. Campus validates the
  // uploaded attachment after the modal is submitted.
  if (activity === 'assignment') {
    const assignmentUpload = new FileUploadBuilder()
      .setCustomId('assignment_upload')
      .setRequired(false)
      .setMinValues(0)
      .setMaxValues(1);
    ...
    modal.addLabelComponents(
      new LabelBuilder()
        .setLabel('Assignment Photo (optional)')
        .setDescription('Upload one PNG, JPG, GIF, or WEBP image of the assignment.')
        .setFileUploadComponent(assignmentUpload)
    );
  }
...
async function handleActivityModal(interaction) {
  ...
  let assignmentUpload = null;
  if (activity === 'assignment') {
    const uploadedFiles = interaction.fields.getUploadedFiles('assignment_upload', false);
    assignmentUpload = uploadedFiles?.first?.() ?? null;

    if (assignmentUpload && !isImageAttachment(assignmentUpload)) {
      return respondInteraction(interaction, ephemeral(
        'The assignment upload must be an image file: PNG, JPG/JPEG, GIF, or WEBP.'
      ));
    }
  }
  ...
  if (activity === 'final') {
    ...
    const { error } = await supabase.from('student_activities').insert({
      ...
      extra_data: {
        ...
        ...(assignmentUpload ? {
          assignment_upload: {
            url: assignmentUpload.url,
            name: assignmentUpload.name || null,
            content_type: assignmentUpload.contentType || null,
            size: assignmentUpload.size || null,
          },
        } : {}),
      },
    });
  ...
  if (assignmentUpload) {
    embed.addFields({
      ...
      value: `[View uploaded photo](${assignmentUpload.url})`,
    });
    embed.setImage(assignmentUpload.url);
  }
  ...
  const uploadText = assignmentUpload
    ? '\n📸 Assignment photo attached and saved with this activity.'
    : '';

  return respondInteraction(
    interaction,
    ephemeral(`✅ **${studentName}** was posted in <#${route.discord_channel_id}>.\n${creditText}${uploadText}`)
  );
}
...
client.once(Events.ClientReady, async (readyClient) => {
  ...
  console.log(aiEnabled()
    ? `AI teachers + NPC students enabled with ${process.env.OPENAI_MODEL || 'gpt-5.6'}.`
    : 'AI teachers are currently OFF because OPENAI_API_KEY is not set.');

  const hasRoutesCommand = commandJSON.some((command) => command.name === 'routes');
  console.log(`[Commands] /routes present in loaded commands.js: ${hasRoutesCommand ? 'YES' : 'NO'}`);
});


function logInteractionState(label, interaction) {
  console.log(`[InteractionState] ${label}`, {
    id: interaction.id,
...
client.on(Events.InteractionCreate, async (interaction) => {
  try {
    ...
    if (interaction.isButton()) {
      ...
      if (interaction.customId.startsWith('campus_teacher_photo_add:')) {
        logInteractionState('teacher-photo-button', interaction);
        return await openTeacherPhotoUpload(interaction);
      }
      if (interaction.customId.startsWith('campus_teacher_photo_skip:')) {
        return await skipTeacherPhoto(interaction);
      }
      if (interaction.customId.startsWith('campus_teacher_photo_remove:')) {
        return await removeTeacherPhoto(interaction);
      }
      ...
      if (interaction.customId.startsWith('campus_npc_personal_open:')) {
        return await openNpcPersonalDetails(interaction);
      }
      if (interaction.customId.startsWith('campus_npc_roaming_open:')) {
        return await openNpcRoaming(interaction);
      }
    }

    if (interaction.isModalSubmit()) {
      // Do NOT globally defer modal submissions here.
      // Each modal handler owns its acknowledgement. This prevents 40060
      // when a handler needs to open another modal or has already responded.
      ...
      if (interaction.customId === 'campus_teacher_schedule') {
        return await handleTeacherScheduleModal(interaction);
      }

      if (interaction.customId === 'campus_teacher_schedule_manage') {
        return await handleTeacherScheduleManageModal(interaction);
      }
      ...
      if (interaction.customId.startsWith('campus_teacher_photo_modal:')) {
        logInteractionState('teacher-photo-modal-submit', interaction);
        return await handleTeacherPhotoUploadModal(interaction);
      }

      if (interaction.customId === 'campus_npc_create') {
        return await handleNpcCreateModal(interaction);
      }
      if (interaction.customId === 'campus_npc_edit') {
        return await handleNpcEditModal(interaction);
      }
      if (interaction.customId === 'campus_npc_status') {
        return await handleNpcStatusModal(interaction);
      }
      if (interaction.customId === 'campus_npc_automation') {
        return await handleNpcAutomationModal(interaction);
      }
      if (interaction.customId === 'campus_npc_remove') {
        return await handleNpcRemoveModal(interaction);
      }
      if (interaction.customId.startsWith('campus_npc_personal:')) {
        return await handleNpcPersonalDetailsModal(interaction);
      }
      if (interaction.customId.startsWith('campus_npc_roaming:')) {
        return await handleNpcRoamingModal(interaction);
      }
...
function extractRoleplayParts(content) {
  const raw = String(content || '');

  // Campus RP convention:
  // **bold** = spoken dialogue
  // plain text = spoken dialogue
  // *italics* = action or internal thought
  const spoken = [];
  const italic = [];

  // Capture bold speech first.
  for (const match of raw.matchAll(/\*\*([\s\S]*?)\*\*/g)) {
    const value = String(match[1] || '').trim();
    if (value) spoken.push(value);
  }

  // Remove bold sections before parsing single-asterisk italics.
  const withoutBold = raw.replace(/\*\*[\s\S]*?\*\*/g, ' ');

  for (const match of withoutBold.matchAll(/(^|[^*])\*([^*\n][\s\S]*?)\*(?!\*)/g)) {
    const value = String(match[2] || '').trim();
    if (value) italic.push(value);
  }

  // Whatever remains outside BOTH bold and italic formatting is ordinary
  // spoken dialogue too. This lets normal Discord messages work naturally.
  const plainSpeech = withoutBold
    .replace(/(^|[^*])\*([^*\n][\s\S]*?)\*(?!\*)/g, '$1 ')
    .replace(/\s+/g, ' ')
    .trim();

  if (plainSpeech) spoken.push(plainSpeech);

  return { spoken, italic };
}

function isPrivateThoughtText(text) {
  const value = String(text || '').trim().toLowerCase();
  if (!value) return false;

  // Explicit internal-thought language. These segments are intentionally
  // hidden from teachers even though the Discord user can see them.
  const thoughtPatterns = [
    /\b(thinks?|thought|thinking)\b/,
    /\b(wonders?|wondered|wondering)\b/,
    /\b(in (?:my|his|her|their) (?:head|mind))\b/,
    /\b(to (?:myself|himself|herself|themselves))\b/,
    /\b(internally|mentally)\b/,
    /\b(hopes?|hoped|wishes?|wished|imagines?|imagined)\b/,
    /\b(reminds? (?:myself|himself|herself|themselves))\b/,
    /\b(i|he|she|they)\s+(?:think|wonder|hope|wish|imagine|know|believe|suppose)\b/,
  ];

  return thoughtPatterns.some((pattern) => pattern.test(value));
}

function roleplayContextForTeacher(content) {
  const parts = extractRoleplayParts(content);
  const actions = parts.italic.filter((segment) => !isPrivateThoughtText(segment));

  return {
    spoken: parts.spoken,
    actions,
    hasPrivateThoughts: parts.italic.length > actions.length,
  };
}

function observableBehaviorNeedsTeacherIntervention(actions) {
  const text = (actions || []).join(' ').toLowerCase();
  if (!text) return false;

  // Observable classroom/school behaviors that can reasonably require a
  // teacher to step in even when nobody verbally addresses them.
  const behaviorPatterns = [
    /\b(hit|hits|hitting|punch|punches|punched|kick|kicks|kicked|slap|slaps|slapped)\b/,
    /\b(shove|shoves|shoved|push|pushes|pushed|fight|fights|fighting)\b/,
    /\b(throw|throws|threw|toss|tosses|fling|flings)\b/,
    /\b(yell|yells|yelled|scream|screams|screamed|shout|shouts|shouted)\b/,
    /\b(curse|curses|cursed|swear|swears|swore)\b/,
    /\b(steal|steals|stole|cheat|cheats|cheating|copies? (?:answers?|homework|work))\b/,
    /\b(vape|vapes|vaping|smoke|smokes|smoking)\b/,
    /\b(break|breaks|broke|smash|smashes|destroy|destroys|damage|damages)\b/,
    /\b(weapon|knife|gun)\b/,
    /\b(flips? (?:him|her|them|the teacher|mr\.?|mrs\.?|ms\.?) off|middle finger)\b/,
    /\b(rolls? (?:his|her|their) eyes at|mocks?|taunts?)\b/,
    /\b(runs? out|storms? out|leaves? (?:the )?(?:class|classroom) without)\b/,
    /\b(climbs? on (?:the )?(?:desk|table|chair)|stands? on (?:the )?(?:desk|table|chair))\b/,
  ];

  return behaviorPatterns.some((pattern) => pattern.test(text));
}

function spokenDialogueAddressesTeacher(spokenLines, teacher) {
  return (spokenLines || []).some((line) => teacherNameMentioned(line, teacher));
}

async function isReplyToTeacherMessage(message, teacher) {
  const repliedId = message?.reference?.messageId;
  if (!repliedId) return false;

  const replied = await message.channel.messages.fetch(repliedId).catch(() => null);
  if (!replied) return false;

  return isOwnTeacherWebhookMessage(replied, teacher);
}

function teacherVisibleRoleplayText(content) {
  const ctx = roleplayContextForTeacher(content);
  const pieces = [];

  if (ctx.spoken.length) {
    pieces.push(`SPOKEN OUT LOUD: ${ctx.spoken.join(' | ')}`);
  }
  if (ctx.actions.length) {
    pieces.push(`OBSERVABLE ACTIONS: ${ctx.actions.join(' | ')}`);
  }

  // Private thoughts are intentionally excluded.
  return pieces.join('\n');
}
...
async function chooseRoamingTeacherForStudentMessage(school, message) {
  ...
  const rp = roleplayContextForTeacher(message.content);

  // Plain or bold spoken dialogue naming a teacher chooses that specific teacher.
  const named = usable.find(({ teacher }) =>
    spokenDialogueAddressesTeacher(rp.spoken, teacher)
  );
  if (named) return named;

  // A reply to a Campus teacher webhook chooses that teacher.
  for (const candidate of usable) {
    if (await isReplyToTeacherMessage(message, candidate.teacher)) return candidate;
  }

  // Otherwise teachers do NOT jump into student conversations.
  // The only exception is an observable action that reasonably requires staff
  // intervention/reprimand; choose one present roaming teacher to handle it.
  if (observableBehaviorNeedsTeacherIntervention(rp.actions)) {
...
client.on(Events.MessageCreate, async (message) => {
  ...
  // Teacher photo uploads are handled by the dedicated per-channel collector.


  if (!aiEnabled() || !message.content) return;
  ...
  try {
    ...
    // One Discord message may only trigger one teacher AI generation.
    // Claim it before calling OpenAI so duplicate events cannot double-charge.
    if (!claimTeacherMessage(teacher.id, message.id)) return;

    // Remember the newest real source message this teacher is processing.
    lastTeacherSourceMessage.set(teacher.id, {
      id: message.id,
    ...
    const rp = roleplayContextForTeacher(message.content);
    const spokenToTeacher = spokenDialogueAddressesTeacher(rp.spoken, teacher);
    const repliedToTeacher = await isReplyToTeacherMessage(message, teacher);
    const needsIntervention = observableBehaviorNeedsTeacherIntervention(rp.actions);

    // Teachers only answer when:
    // 1) plain or bold spoken dialogue is actually directed at them,
    // 2) the student directly replies to that teacher's webhook message, or
    // 3) an observable action requires staff intervention/reprimand.
    //
    // Private thoughts are NEVER treated as speech and are never passed to AI.
    if (!spokenToTeacher && !repliedToTeacher && !needsIntervention) return;
    ...
    if (quiet) {
      ...
      if (!allowDirect && !needsIntervention) return;
    }
    ...
    const recent = [...fetched.values()]
      ...
      .map((m) => {
        const visible = teacherVisibleRoleplayText(m.content);
        return visible ? `${m.member?.displayName || m.author.username}: ${visible}` : null;
      })
      .filter(Boolean);

    if (teacherReplyInFlight.has(teacher.id)) return;
    teacherReplyInFlight.add(teacher.id);

    // Reserve the teacher immediately. This prevents a spontaneous tick from
    // starting another paid AI request while this student reply is generating.
    ...
    try {
      const decision = await teacherConversationDecision({
        ...
        triggeringMessage:
          `${teacherVisibleRoleplayText(message.content)}\n`
          + (needsIntervention
            ? 'SYSTEM CONTEXT: An observable action may require a teacher reprimand or safety intervention. Respond only if appropriate.'
            : 'SYSTEM CONTEXT: The teacher was directly addressed through spoken dialogue or a direct reply.'),
        spontaneous: false,
      ...
      teacherReplyInFlight.delete(teacher.id);
    }
  } catch (error) {
    console.error('Conversation awareness error:', error);
  }
});


function npcAsTeacherProfile(npc) {
  return {
    id: npc.id,
    teacher_name: npc.student_name,
    subject: `Student / ${npc.grade_level || 'School'}`,
    grade_level: npc.grade_level,
    personality:
      `IMPORTANT ROLE RULE: You are a STUDENT at this school, NOT a teacher or staff member. `
      + `Never give teacher-like instructions, never assign work, and never speak as an authority figure. `
      + `Act naturally like a school-age peer. Personality: ${npc.personality || 'friendly and realistic'}.`,
    teaching_style:
      `Student behavior only. Interests/Hobbies: ${npc.interests || 'not specified'}. `
      + `Personal details: ${npc.lore || 'none specified'}. Keep replies conversational and age-appropriate for ${npc.grade_level || 'their grade'}.`,
    lore: npc.lore,
    strictness: 2,
    talkativeness: 6,
    humor: 5,
...
function npcNameMentioned(content, npc) {
  const text = String(content || '').toLowerCase();
  const full = String(npc.student_name || '').trim().toLowerCase();
  if (!full) return false;
  const parts = full.split(/\s+/).filter((part) => part.length >= 2);
  return text.includes(full) || parts.some((part) => text.includes(part));
}

function isOwnNpcWebhookMessage(message, npc) {
  return Boolean(message?.webhookId)
    && String(message.author?.username || '').trim().toLowerCase()
      === String(npc.student_name || '').trim().toLowerCase();
}

async function sendAsNpc(channel, npc, content) {
  if (!content) return;
  try {
    const webhookChannel = channel.isThread?.() ? channel.parent : channel;
    if (!webhookChannel?.fetchWebhooks) throw new Error('No webhook-capable parent channel.');
    const webhooks = await webhookChannel.fetchWebhooks();
    let webhook = webhooks.find((w) => w.owner?.id === client.user.id && w.name === 'Campus NPC Students');
    if (!webhook) {
      webhook = await webhookChannel.createWebhook({
        name: 'Campus NPC Students',
        reason: 'Campus NPC student messages',
      });
    }
    await webhook.send({
      content,
      username: npc.student_name,
      avatarURL: npc.avatar_url || undefined,
      threadId: channel.isThread?.() ? channel.id : undefined,
      allowedMentions: { parse: [] },
    });
  } catch (error) {
    console.warn('NPC webhook send failed; using Campus fallback:', error.message);
    await channel.send({ content: `**${npc.student_name}:** ${content}`, allowedMentions: { parse: [] } });
  }
}

async function getNpcCandidatesForChannel(schoolId, channelId) {
  ...
  const { data: settingsRows, error: settingsError } = await supabase
    ...
    .in('npc_id', ids);
  if (settingsError) throw settingsError;
  const settingsMap = new Map((settingsRows ?? []).map((row) => [String(row.npc_id), row]));

  return npcs.map((npc) => ({ npc, settings: settingsMap.get(String(npc.id)) || null }))
    .filter(({ npc, settings }) => {
      if (!settings?.automation_enabled || !settings?.conversation_awareness || !settings?.respond_to_messages) return false;
      if (String(npc.assigned_channel_id) === String(channelId)) return true;
      return Boolean(settings.roaming_enabled)
        && Array.isArray(settings.roaming_channel_ids)
        && settings.roaming_channel_ids.map(String).includes(String(channelId));
    });
}

const npcFrequencyChance = {
  very_low: 0.03,
  low: 0.07,
  normal: 0.15,
  high: 0.30,
...
const npcRoamingChance = { rare: 0.05, normal: 0.12, frequent: 0.25 };
...
client.on(Events.MessageCreate, async (message) => {
  ...
  try {
    const school = await getSchool(message.guild.id);
    if (!school) return;
    const candidates = await getNpcCandidatesForChannel(school.id, message.channel.id);
    if (!candidates.length) return;

    // Never let an NPC react to its own webhook output.
    const usable = candidates.filter(({ npc }) => !isOwnNpcWebhookMessage(message, npc));
    if (!usable.length) return;

    // A named NPC gets first choice. Otherwise choose only ONE candidate so a
    // normal message does not make every NPC in the room answer at once.
    let chosen = usable.find(({ npc }) => npcNameMentioned(message.content, npc));
    if (!chosen) {
      const shuffled = [...usable].sort(() => Math.random() - 0.5);
      for (const candidate of shuffled) {
        const isRoaming = String(candidate.npc.assigned_channel_id) !== String(message.channel.id);
        const base = isRoaming
          ? (npcRoamingChance[candidate.settings?.roaming_frequency] ?? npcRoamingChance.normal)
          : (npcFrequencyChance[candidate.settings?.activity_frequency] ?? npcFrequencyChance.normal);
        if (Math.random() <= base) {
          chosen = candidate;
          break;
        }
    ...
    const { npc, settings } = chosen;
    const directlyAddressed = npcNameMentioned(message.content, npc);
    if (!directlyAddressed) {
      const cooldownMs = (settings.minimum_cooldown_minutes ?? 30) * 60_000;
      if (Date.now() - (lastNpcAction.get(npc.id) || 0) < cooldownMs) return;
    }

    const fetched = await message.channel.messages.fetch({ limit: 12 });
    const recent = [...fetched.values()]
      .reverse()
      .filter((m) => m.content && !isOwnNpcWebhookMessage(m, npc))
      .filter((m) => !m.author.bot || Boolean(m.webhookId))
      .map((m) => `${m.member?.displayName || m.author.username}: ${m.content}`)
      .filter(Boolean);

    const decision = await teacherConversationDecision({
      teacher: npcAsTeacherProfile(npc),
      recentMessages: recent,
      triggeringMessage: message.content,
      spontaneous: false,
      setting: String(npc.assigned_channel_id) === String(message.channel.id)
        ? `school channel #${message.channel.name}`
        : `common area #${message.channel.name}`,
    });

    if (decision.type === 'react' && settings.react_to_messages) {
      await message.react(decision.emoji).catch(() => {});
      lastNpcAction.set(npc.id, Date.now());
    } else if (decision.type === 'reply') {
      await sendAsNpc(message.channel, npc, decision.content);
      lastNpcAction.set(npc.id, Date.now());
    }
  } catch (error) {
    console.error('NPC conversation awareness error:', error);
  }
});

async function spontaneousNpcTick() {
  if (!aiEnabled()) return;
  const { data: npcs, error } = await supabase
    .from('npc_students')
    .select('*')
    .eq('status', 'active');
  if (error) {
    console.error('NPC spontaneous load error:', error.message);
    return;
  }
  ...
  for (const npc of npcs ?? []) {
    try {
      const { data: settings, error: settingsError } = await supabase
        ...
        .maybeSingle();
      if (settingsError) throw settingsError;
      if (!settings?.automation_enabled || !settings.spontaneous_messages) continue;

      const cooldownMs = (settings.minimum_cooldown_minutes ?? 30) * 60_000;
      if (Date.now() - (lastNpcAction.get(npc.id) || 0) < cooldownMs) continue;

      const history = (npcSpontaneousHistory.get(npc.id) || []).filter((ts) => Date.now() - ts < 3_600_000);
      if (history.length >= 2) continue;

      const chance = npcFrequencyChance[settings.activity_frequency] ?? npcFrequencyChance.normal;
      if (Math.random() > chance) continue;

      const channelIds = [String(npc.assigned_channel_id || '')];
      if (settings.roaming_enabled && Array.isArray(settings.roaming_channel_ids)) {
        channelIds.push(...settings.roaming_channel_ids.map(String));
      }
      const available = channelIds.filter(Boolean).sort(() => Math.random() - 0.5);
      let channel = null;
      for (const channelId of available) {
      ...
      const decision = await teacherConversationDecision({
        ...
        triggeringMessage: '',
        spontaneous: true,
        setting: String(npc.assigned_channel_id) === String(channel.id)
          ? `school channel #${channel.name}`
          : `common area #${channel.name}`,
      });
      if (decision.type !== 'reply') continue;
      await sendAsNpc(channel, npc, decision.content);
      const now = Date.now();
      lastNpcAction.set(npc.id, now);
      npcSpontaneousHistory.set(npc.id, [...history, now]);
    } catch (error) {
      console.error(`Spontaneous NPC ${npc.id} error:`, error.message);
    }
  }
}
...
async function automaticAbsenceTick() {
  ...
  for (const teacher of teachers ?? []) {
    try {
      // Never start spontaneous AI while this teacher is already generating
      // a response to a real Discord message.
      if (teacherReplyInFlight.has(teacher.id)) continue;
...
async function spontaneousTeacherTick() {
  ...
  for (const teacher of teachers ?? []) {
    try {
      ...
      const fetched = await channel.messages.fetch({ limit: 20 }).catch((error) => {
        if (error?.code === 50001 || error?.message === 'Missing Access') return null;
        throw error;
      });
      if (!fetched) continue;
      ...
      // Find the newest message that could actually be a student/Tupper source.
      // Teacher webhook output and ordinary bots do not count.
      const currentTeacherName = String(teacher.teacher_name || '').trim().toLowerCase();
      const eligibleSourceMessages = messages.filter((m) => {
        if (!m.author.bot) return true;
        if (!m.webhookId) return false;
        return String(m.author.username || '').trim().toLowerCase() !== currentTeacherName;
      });

      const newestSource = eligibleSourceMessages.at(-1) || null;
      const alreadyHandled = lastTeacherSourceMessage.get(teacher.id);

      // If there has been no new student/Tupper message since the teacher's last
      // handled source message, do NOT let spontaneous automation revive the old
      // conversation and generate another paid reply.
      if (
        newestSource
        && alreadyHandled
        && newestSource.createdTimestamp <= alreadyHandled.createdTimestamp
      ) {
        continue;
      }
      ...
      if (newestSource) {
        lastTeacherSourceMessage.set(teacher.id, {
          id: newestSource.id,
          createdTimestamp: newestSource.createdTimestamp,
        });
      }
...
setInterval(async () => {
  ...
  await spontaneousNpcTick();
}, tickMinutes * 60_000);

// Scheduled teacher announcements need minute-level precision.
setInterval(async () => {
  await teacherScheduleTick();
}, 60_000);
