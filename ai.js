...
export async function teacherConversationDecision({ teacher, recentMessages, triggeringMessage, spontaneous = false, setting = 'assigned classroom' }) {
...
Current school setting: ${setting}
Recent messages in this space:
${recentMessages.slice(-12).map((m) => `- ${m}`).join('\n') || '- No recent conversation.'}

${spontaneous
  ? `Decide whether this teacher would naturally say something on their own right now. In common areas, the teacher may casually talk with students or respond to another teacher if that fits the profile and recent messages. Never write dialogue for anyone except this teacher.
Return exactly one of:
SILENCE
REPLY: <short teacher message>`
  : `A new message arrived in this school space:
"${triggeringMessage}"
