...
async function handleTeacherRoamingModal(interaction) {
  ...
  const channelIds = [...selectedChannels.values()].map((channel) => String(channel.id));
  ...
  if (enabled && channelIds.length === 0) {
    return interaction.reply(ephemeral('Choose at least one common-area channel before turning roaming ON.'));
  }
...
async function handleTeacherAbsenceModal(interaction) {
  ...
  if (coverage === 'saved' && !savedSubId) {
    return interaction.reply(ephemeral('Choose a teacher under **Preferred Saved Substitute**, or pick Generic Substitute instead.'));
  }
...
async function handleTeacherAutomationResetModal(interaction) {
  ...
  if (!scope || !action) {
    return interaction.reply(ephemeral('Choose a teacher and a reset option.'));
  }

  const teacherIds = await getTeacherIdsForAutomationScope(school.id, scope);
  if (!teacherIds.length) {
    return interaction.reply(ephemeral('I could not find any teachers for that reset.'));
  }
  ...
  return interaction.reply(ephemeral('Unknown automation reset option.'));
}
...
async function openTeacherAutomationRedo(interaction) {
  ...
  const teacherIds = await getTeacherIdsForAutomationScope(school.id, scope);
  if (!teacherIds.length) {
    return interaction.reply(ephemeral('I could not find any teachers for that automation setup.'));
  }
...
async function handleTeacherList(interaction) {
  ...
  if (!teachers?.length) {
    return interaction.reply(ephemeral('No teachers are saved for this Discord yet. Use `/teacher create` first.'));
  }
  ...
  await interaction.reply(ephemeral(chunks[0]));
...
async function handleGradeAll(interaction) {
  ...
  if (!names.length) {
    return interaction.reply(ephemeral('📊 No students have logged Campus activities yet.'));
  }
  ...
  for (const name of names) {
    const rows = await getProgressData(school, name);
    if (!rows.length) {
      return interaction.reply(ephemeral('No quarters are configured yet. An admin needs to run `/quarter setup`.'));
    }
  ...
  await interaction.reply(ephemeral(pages[0]));
...
async function openGradeResetModal(interaction) {
  if (!interaction.memberPermissions?.has('ManageGuild')) {
    return interaction.reply(ephemeral('Only school admins can reset grades.'));
  }
  ...
  if (!quarterResult.data?.length) {
    return interaction.reply(ephemeral('No quarters are configured yet. Run `/quarter setup` first.'));
  }

  if (!students.length) {
    return interaction.reply(ephemeral('There are no tracked students to reset yet.'));
  }
...
async function handleGradeResetModal(interaction) {
  ...
  if (confirm !== 'yes') {
    return interaction.reply(ephemeral('Grade reset cancelled.'));
  }
  ...
  if (!quarterIds.length) {
    return interaction.reply(ephemeral('No matching quarter was found.'));
  }
  ...
  if (mode === 'progress_only') {
    ...
    return interaction.reply(ephemeral('Unknown grade reset type.'));
  }
...
async function handleGradeCommand(interaction) {
  ...
  return interaction.reply(ephemeral('Choose `/grade student`, `/grade all`, or `/grade reset`.'));
}
      ...
      if (isCommonArea) {
        recent = messages
          ...
          .filter((m) => {
            ...
            const isOtherCampusTeacher = Boolean(m.webhookId)
              && knownTeacherNames.has(String(m.author.username || '').toLowerCase())
              && String(m.author.username || '').toLowerCase() !== String(teacher.teacher_name || '').toLowerCase();
