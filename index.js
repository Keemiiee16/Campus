...
async function openTeacherPhotoUpload(interaction) {
  ...
  const uploadChannel = interaction.channel;
  if (!uploadChannel?.awaitMessages) {
    console.error('Teacher photo collector error: channel does not support awaitMessages');
    return;
  }
  ...
  try {
    const collected = await uploadChannel.awaitMessages({
      filter,
      max: 1,
      time: 5 * 60_000,
      errors: ['time'],
    ...
    const attachment = uploadMessage?.attachments?.find((item) => isImageAttachment(item));
    if (!attachment) return;

    const { data: refreshedTeacher, error: teacherError } = await supabase
      .from('teachers')
    ...
    if (!refreshedTeacher) throw new Error('Teacher no longer exists.');

    const { error: updateError } = await supabase
      .from('teachers')
      .update({ avatar_url: attachment.url })
      .eq('id', refreshedTeacher.id);
    if (updateError) throw updateError;

    console.log(`Teacher photo saved for ${refreshedTeacher.teacher_name}: ${attachment.url}`);

    await uploadMessage.reply({
      content: `✅ **${refreshedTeacher.teacher_name}'s photo is saved!** Their future teacher messages will use it.`,
      allowedMentions: { repliedUser: false },
    }).catch(() => {});
  } catch (collectorError) {
    if (collectorError?.constructor?.name === 'Collection') return;
    console.error('Teacher photo collector error:', collectorError);
  }
}
